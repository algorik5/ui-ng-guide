...
export class AtableTestComponent implements OnInit {
  ...
  ngOnInit() {
    this.pubsub.sub(this.myname+".table.selectdata", datas => {
    });
  }
  ...
  test_datas()
  {
    ...
    this.pubsub.pub(this.myname+".table.datas",datas);//this.table.setData(datas);
  }
  test_no = 0;
  test_data() { 
    ...
    this.pubsub.pub(this.myname+".table.data",data);//this.table.addDatas(data);
  }
  test_columnshow()
  {
    this.pubsub.pub(this.myname+".table.columnshow","host");//this.table.changeColumnShow("host");
  }
  test_editable = false; 
  test_edit() { 
    this.test_editable = this.test_editable?false:true; 
    this.pubsub.pub(this.myname+".table.editable",this.test_editable);//this.table.changeColumnShow("host");
  }
  test_checkable = false; 
  test_check() { 
    this.test_checkable = this.test_checkable?false:true; 
    this.pubsub.pub(this.myname+".table.checkable",this.test_checkable);//this.table.changeColumnShow("host");
  }
  test_checkedall = false; 
  test_checkall() { 
    this.test_checkedall = this.test_checkedall?false:true; 
    this.pubsub.pub(this.myname+".table.checkall",this.test_checkedall);//this.table.changeColumnShow("host");
  }
